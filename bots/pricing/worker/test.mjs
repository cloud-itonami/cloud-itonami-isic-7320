...
for(const failure of [null,'model','storage','readback'])test('market workflow '+failure,async()=>{
 ...
 try{const actor=new PricingResident({storage:{put:async(k,v)=>rows.set(k,v),get:async k=>rows.get(k)}},{MURAKUMO_API_KEY:'fixture',KOTOBASE_ARCHIVE_TOKEN:'fixture'});await actor.review(actor.env);const status=rows.get('status');assert.equal(status.state,failure?'failed':'completed');assert.equal(rows.has('lastSuccess'),!failure);if(failure)assert.match(status.error,{model:/invalid-calculation/,storage:/kotobase-write-503/,readback:/kotobase-readback-mismatch/}[failure]);else assert.equal(status.storage.readbackVerified,true);}finally{globalThis.fetch=old;}
});
