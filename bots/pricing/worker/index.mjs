...
export class PricingResident extends DurableObject {
 ...
 async fetch(request) {
  if(new URL(request.url).pathname==='/review') {await this.review(this.env);return Response.json(await this.ctx.storage.get('status'));}
...
export default {
 ...
 async fetch(request,env){const url=new URL(request.url);if(request.method!=='GET'||url.pathname!=='/status')return new Response('Not found',{status:404});return env.PRICING.getByName('pricing').fetch('https://internal/status');}
};
