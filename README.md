...
| File | Role |
|---|---|
| `src/polling/store.cljk` | **Store** protocol -- `MemStore` ‖ `DatomicStore` (`langchain.db`) + append-only audit ledger + findings-report history. No dynamically-filed sub-record -- the actuation op acts directly on a pre-seeded survey, and the double-actuation guard checks a dedicated `:findings-report-published?` boolean rather than a `:status` value |
| `src/polling/registry.cljk` | Findings-report draft records, plus `sample-size-insufficient?` -- the SIXTH instance of this fleet's MINIMUM-threshold sufficiency check family (`veterinary`/`funeral`/`hospital` established the first three, all temporal; `association`/`secondary` generalized it to non-temporal ground truths as the fourth and fifth) |
| `src/polling/facts.cljk` | Per-jurisdiction market-research-professional-standards catalog with an official spec-basis citation per entry, honest coverage reporting. **CHN carries an explicit `:out-of-scope-here`**: China's foreign-related survey regime (涉外调查管理办法 — the 涉外调查许可证, and the per-project approval that 涉外社会调查 but *not* 涉外市场调查 additionally needs) is a conditional gate whose applicability depends on a per-survey classification, so [`cloud-itonami-iso3166-chn-market-research`](https://github.com/cloud-itonami/cloud-itonami-iso3166-chn-market-research) implements it and this row names the boundary rather than implying full coverage |
| `src/polling/pollingadvisor.cljk` | **PollOps-LLM** -- `mock-advisor` ‖ `llm-advisor`; intake/methodology-verification/unrepresentative-sample-risk-screening/findings-report-publication proposals |
| `src/polling/governor.cljk` | **Survey Integrity Governor** -- 3 HARD checks (spec-basis · evidence-incomplete · sample-size-insufficient, pure ground-truth floor recompute · unrepresentative-sample-risk-unresolved, unconditional evaluation, the THIRTY-NINTH grounding of this discipline, a genuinely new concept grounded in this blueprint's own Trust Control text) + already-published guard + 1 soft (confidence/actuation gate) |
| `src/polling/phase.cljk` | **Phase 0→3** -- read-only → assisted intake → assisted verify → supervised (findings-report publication always human; survey intake is the ONLY auto-eligible op, no direct capital risk) |
| `src/polling/operation.cljk` | **OperationActor** -- langgraph-clj StateGraph |
| `src/polling/sim.cljk` | demo driver |
