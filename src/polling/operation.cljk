(ns polling.operation
  "OperationActor -- one polling operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (PollOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Survey Integrity Governor (:govern) and the
  ...
  One graph run = one polling operation (intake -> advise -> govern ->
  ...
  decision to a human operator (research operator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/publish-findings-report` ALWAYS reaches this node when
  the governor is clean -- see `polling.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [polling.pollingadvisor :as pollingadvisor]
            [polling.governor :as governor]
...
(defn build
  ...
  `polling.store/Store`).
  opts:
    :advisor      -- a `polling.pollingadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (pollingadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PollOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (pollingadvisor/-advise advisor store request)]
            {:proposal p :audit [(pollingadvisor/trace request p)]})))

      ;; Survey Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
